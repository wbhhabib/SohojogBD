...
type VerifyFilter = 'all' | 'VERIFIED' | 'PENDING' | 'REJECTED' | 'NOT_SUBMITTED'
...
export default function AdminUsersPage() {
  ...
  const fetchUsers = useCallback(() => {
    ...
    if (verifyFilter !== 'all') params.set('verificationStatus', verifyFilter)
  ...
  }, [search, roleFilter, statusFilter, verifyFilter, page])

  useEffect(() => { fetchUsers() }, [fetchUsers])

  const handleSearch = (val: string) => { setSearch(val); setPage(1) }
  const handleRoleFilter = (val: RoleFilter) => { setRoleFilter(val); setPage(1) }
  const handleVerifyFilter = (val: VerifyFilter) => { setVerifyFilter(val); setPage(1) }
      ...
      <DashboardLayout role="admin">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-xl flex-wrap">
            {(['all', 'USER', 'ADMIN'] as RoleFilter[]).map((r) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize whitespace-nowrap ${roleFilter === r ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
              >
          ...
          <div className="flex gap-1 bg-gray-100 p-1 rounded-xl flex-wrap">
            ...
            ] as { value: VerifyFilter; label: string }[]).map((opt) => (
              <button
                key={opt.value}
                onClick={() => handleVerifyFilter(opt.value)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap ${verifyFilter === opt.value ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                  }`}
              >
                {opt.label}
              </button>
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <div>
                                ...
                                {(!u.verificationStatus || u.verificationStatus === 'NOT_SUBMITTED') && (
                                  <span className="text-[10px] text-slate-400 font-medium">Not verified</span>
                                )}
                              </div>
                            </div>
                          </td>
